!function(window, undefined) {
...
}(window, window.kg), function(exports, $, kg) {
    "use strict";
...
}(window, window.$, window.kg), function(exports, $, kg) {
    "use strict";
    function Scope(options) {
        kg.extend(this, options);
    }
    "undefined" != typeof kg && (Scope.prototype = {
        watch: function(prop, fn) {
            kg.watch(this, prop, fn);
        },
        set: function(key, value) {
            kg.isFunction(key) ? key() : kg.isDefined(key) && kg.isDefined(value) && (this[key] = value), 
            kg.apply();
        }
    }, kg.Scope = Scope);
}(window, window.$, window.kg), function(exports, $, kg) {
    "use strict";
    function Eventer() {}
    "undefined" != typeof kg && (Eventer.prototype = {
        constructor: Eventer,
        on: function(evt, fn, ctx) {
            var shouldAdd = !0, ctx = ctx || this, data = null;
            return this.hasOwnProperty("subscribers") || (this.subscribers = {}), this.subscribers[evt] || (this.subscribers[evt] = []), 
            shouldAdd && (data = {
                fn: fn,
                ctx: ctx
            }, this.subscribers[evt].push(data)), this;
        },
        off: function(evt, fn) {
            var evtSubscribers = this.subscribers[evt] || null, index = -1, i = 0;
            if (evtSubscribers && evtSubscribers.length > 0) if (kg.isFunction(fn)) evtSubscribers = []; else for (i = 0; i < evtSubscribers.length; i++) if (evtSubscribers[i].fn === fn) {
                index = i;
                break;
            }
            return index > -1 && evtSubscribers.splice(index, 1), this;
        },
        trigger: function(evt) {
            var evtSubscribers = null, i = 0, len = 0, evtData = null, temp = null;
            if (this.hasOwnProperty("subscribers") && (evtSubscribers = this.subscribers[evt] || null, 
            evtSubscribers && evtSubscribers.length > 0)) for (i = 0, len = evtSubscribers.length; len > i; i++) temp = evtSubscribers[i], 
            evtData = {}, evtData.target = this, evtData = kg.extend(evtData, temp.data), temp.fn.call(temp.ctx, evtData);
            return this;
        },
    ...
    "use strict";
    function View() {}
    "undefined" != typeof kg && (View.prototype = new kg.Eventer(), View.prototype.extend({
        constructor: View,
        find: function(selector) {
    ...
    "use strict";
    "undefined" != typeof kg && (kg.musicplayer = {}, $(function() {
        var songs = [], activeIndex = 0, scope = new kg.Scope({
            duration: "00:00",
            currentTime: "00:00",
            seek: 0,
            progress: 0,
        ...
        $(".kg-track-entry").each(function() {
            ...
            var song = null, i = 0, len = songs.length;
            if (oldSong && !newSong) activeIndex = activeIndex++, activeIndex > songs.length - 1 ? activeIndex = 0 : (song = songs[activeIndex], 
            scope.set("activeSong", song)); else for (i = 0; len > i; i++) if (songs[i] === newSong) {
                activeIndex = i;
                break;
            }
        });
    ...
    function Song(options) {
        kg.log("Song"), this.$el = $(options.element), this.scope = options.scope, this.audio = null, 
        this.genre = this.attr("data-track-genre"), this.$duration = this.find(".duration"), 
        this.isPlaying = !1, this.init.call(this);
    }
    "undefined" != typeof kg && (Song.prototype = new kg.View(), Song.prototype.extend({
        constructor: Song,
        init: function() {
            var self = this;
            return this.prepareAudio(), this.$el.on("click", this.handleClick.bind(this)), self.scope.watch("activeSong", function(newSong) {
                newSong && newSong === self ? newSong && newSong === self && self.playSong() : self.pauseSong();
            }), self.scope.watch("currentTime", function(newTime) {
                self.$duration.html(newTime && self.isPlaying ? newTime : "00:00");
            }), self.scope.watch("seek", function(newSeek) {
                newSeek && self.isPlaying && self.audio.seek(newSeek);
            }), this;
        },
        prepareAudio: function() {
            kg.sniffer.audio && (this.audio = new kg.musicplayer.Audio({
                element: this.find("audio").get(0),
                scope: this.scope
            }));
        },
        handleClick: function() {
            kg.sniffer.audio ? this.isPlaying ? this.pauseSong() : this.playSong() : alert("Your browser does not support mp3 audio. Please try the latest version of Google Chrome, Apple Safari, Internet Explorer or Mozilla Firefox on Windows.");
        },
        playSong: function() {
            this.isPlaying || (this.scope.set("activeSong", this), this.$el.addClass("song_playing"), 
            this.audio.play(), this.isPlaying = !0);
        },
        pauseSong: function() {
            this.isPlaying && (this.$el.removeClass("song_playing"), this.audio.pause(), this.isPlaying = !1);
        },
        setActive: function() {
            this.isActive = !0, this.$el.addClass("genre_active");
        },
        setInactive: function() {
            this.isActive && (this.isActive = !1, this.$el.removeClass("genre_active"));
        }
    }), kg.musicplayer.Song = Song);
}(window, window.$, kg), function(exports, $, kg) {
    function ProgressBar(options) {
        this.$el = $(options.element), this.scope = options.scope, this.init.call(this);
    }
    "undefined" != typeof kg && (ProgressBar.prototype = new kg.View(), ProgressBar.prototype.extend({
        constructor: ProgressBar,
        init: function() {
            return this.parseHTML(), this.progressWidth = this.$clickBar[0].offsetWidth, this.$clickBar.on("click", this.seekToClick.bind(this)), 
            this.scope.watch("buffer", this.updateBuffer.bind(this)), this.scope.watch("progress", this.updateProgress.bind(this)), 
            this;
        },
        seekToClick: function(evt) {
            evt.preventDefault(), evt.stopPropagation();
            var x = evt.offsetX, seek = 0;
            void 0 === x && (x = evt.clientX - $(evt.target).offset().left), seek = x / this.progressWidth, 
            this.scope.set("seek", seek);
        },
        parseHTML: function() {
            this.$bufferBar = this.find(".buffer-bar"), this.$progressBar = this.find(".progress-bar"), 
            this.$clickBar = this.find(".click-bar");
        },
        updateBuffer: function(newBuffer) {
            this.$bufferBar.css({
                width: newBuffer + "%"
            });
        },
    ...
    function Audio(options) {
        this.$el = $(options.element), this.audio = options.element, this.scope = options.scope, 
        this.init.call(this);
    }
    "undefined" != typeof kg && (Audio.prototype = new kg.Eventer(), Audio.prototype.extend({
        constructor: Audio,
        init: function() {
            return this.$el.on("loadstart", this.loadStart.bind(this)), this.$el.on("progress", this.buffering.bind(this)), 
            this.$el.on("loadedmetadata", this.metaData.bind(this)), this.$el.on("canplay", this.loadedData.bind(this)), 
            this.$el.on("waiting", this.audioWaiting.bind(this)), this.$el.on("stalled", this.audioStalled.bind(this)), 
            this.$el.on("timeupdate", this.timeUpdate.bind(this)), this.$el.on("ended", this.ended.bind(this)), 
            this.$el.on("playing", this.playing.bind(this)), this.$el.on("pause", this.paused.bind(this)), 
            this.$el.on("error", this.audioError.bind(this)), this;
        },
        loadStart: function() {},
        buffering: function() {
            var duration = this.audio.duration, buffered = this.audio.buffered.length > 0 ? this.audio.buffered.end(0) : 0, buffer = buffered / duration * 100;
        ...
        timeUpdate: function() {
            ...
            var currentTime = this.audio.currentTime || 0, duration = this.audio.duration || 0, progress = currentTime / duration * 100;
            this.scope.set(function() {
                self.scope.currentTime = kg.formatTime(currentTime), self.scope.duration = kg.formatTime(duration), 
        ...
        ended: function() {
            var self = this;
            this.scope.set(function() {
                self.scope.activeSong = null, self.scope.currentTime = "00:00", self.scope.duration = "00:00", 
                self.scope.progress = 0, self.scope.buffer = 0;
            });
        },
        playing: function() {},
        paused: function() {},
